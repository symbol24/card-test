...
func _process(delta: float) -> void:
	if grabbing and not grabbed_items.is_empty():
		...
		for each in grabbed_items:
			#tween.parallel()
			tween.tween_property(each, "global_position", get_global_mouse_position()-connection_point, delta+move_delay)
...
func _grab() -> void:
	if grabbed_items.is_empty():
		...
		if not grabbed_items.is_empty():
			...
			new_panel.custom_minimum_size = Vector2.ZERO
			new_panel.set_deferred("size", Vector2.ZERO)
			new_panel.position = get_global_mouse_position()
			new_panel.z_index = Game.get_highest_card_z_index()+1
